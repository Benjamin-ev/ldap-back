...
app.get('/', (req,res) => {
  res.sendFile(path.join(__dirname, './out/index.html')) // Route principale, page d'accueil
})
routes.map(item => 
  app.get('/'+item, (req, res) => {
    res.sendFile(path.join(__dirname, './out/'+item+'.html')) // Différentes routes de l'application, permet le refresh de la page
  })
)
...
app.get('*', (req, res) => 
  res.sendFile(path.join(__dirname, './out/404.html')) // Route 404 si cela ne mène a rien
)
